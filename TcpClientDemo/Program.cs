...
namespace TcpClientDemo
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            try
            {
                var clientIPEndPoint = new IPEndPoint(IPAddress.Parse("192.168.50.105"), 4196); // 当前客户端使用的ip和端口
                tcpClient = new TcpClient(clientIPEndPoint);
                var serverIPEndPoint = new IPEndPoint(IPAddress.Parse("192.168.50.202"), 4196); // 当前服务器使用的ip和端口
